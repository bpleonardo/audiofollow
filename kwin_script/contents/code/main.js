...
function notify(window) {
    ...
    if (!screen) return;
    if (lastOutput[window.pid] === screen.name) {return} else {
        console.debug(`${window.pid} changed screens.`)
    };
    lastOutput[window.pid] = screen.name;

    callDBus(
        DBUS_NAME, DBUS_PATH, DBUS_NAME, "WindowMoved",
        window.pid, screen.name, function () {}
    );
}
...
function track(window) {
    ...
    window.frameGeometryChanged.connect(function () { notify(window); });
    notify(window);
}
...
function init() {
    ...
    var existing = workspace.windowList ? workspace.windowList() : workspace.clientList();
